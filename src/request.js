...
/**
 * 管理端 / 心理中心接口：禁止使用学生端 User_token，避免 JWT 角色不符导致大量 403。
 * 仅使用管理端登录写入的 auth_token、admin_token、access_token。
 */
function getStaffAccessTokenForUrl(urlStr) {
  const u = String(urlStr || '')
  const needStaff =
    u.includes('/api/admin') ||
    u.includes('/api/center') ||
    u.includes('/api/consultant') ||
    u.includes('/api/schedule')
  if (!needStaff) return null

  return (
    normalizeToken(localStorage.getItem(AUTH_TOKEN_STORAGE_KEY)) ||
    normalizeToken(localStorage.getItem('admin_token')) ||
...
request.interceptors.request.use((config) => {
  ...
  let token =
    override != null
      ...
      : ''
  if (override == null) {
    const staff = getStaffAccessTokenForUrl(config.url || '')
    if (staff !== null) {
      token = staff
    } else {
