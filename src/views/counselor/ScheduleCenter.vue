      ...
      <el-dialog v-model="editDialogVisible" title="修改排班">
        <el-form :model="editForm">
          ...
          <el-form-item label="咨询师">
            <el-select v-model="editForm.counselorId" filterable>
...
const editForm = ref({ date: '', time: '', counselorId: '', consultant_college_id: '' })

onMounted(async () => {
  if (!canView.value) return
  const all = await fetchSchedule()
  mySchedule.value = all.filter((s: any) => s.counselorId === userId)
})

function editSlot(row: any) {
  editForm.value = { date: row.date, time: row.time, counselorId: row.counselorId, consultant_college_id: row.consultant_college_id }
  editDialogVisible.value = true
}

async function saveEdit() {
  
   const consultant = consultants.find(c => c.id === editForm.value.counselorId)

    await updateSchedule({
    date: editForm.value.date,
    time: editForm.value.time,
    counselorId: editForm.value.counselorId,
    counselorName: consultant?.name || '',
  })
  const all = await fetchSchedule()
  mySchedule.value = all.filter((s: any) => s.counselorId === userId)
