...
const overviewStats = ref([
  ...
  { label: '处理中', value: 42, icon: Document, color: '#3b82f6', bgColor: '#eff6ff' },
...
const levelDistribution = computed(() => {
  ...
  return [
    ...
    { value: 'blue', label: '关注', count: 50, color: '#3b82f6', percentage: Math.round(50 / total * 100) }
  ]
})
...
const loadData = async () => {
  ...
  try {
    ...
    const res = await request.get('/api/crisis/list', { params })
    if (res.code === 200) {
      crisisList.value = res.data?.list || res.data || []
      pagination.value.total = res.data?.total || crisisList.value.length
    }
  } catch (e) {
    crisisList.value = generateMockData()
    pagination.value.total = 186
  }
  loading.value = false
}
...
const showReportDialog = () => {
  router.push('/admin/tutor-crisis-report')
}
