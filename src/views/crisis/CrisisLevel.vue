...
const loadCrisisDetail = async () => {
  ...
  try {
    const res = await request.get(`/api/crisis/detail`, { params: { id } })
    if (res.code === 200) {
      currentCase.value = res.data
      levelHistory.value = res.data?.levelHistory || []
    }
  } catch (e) {
    currentCase.value = {
      id: id || 'cr_001',
    ...
    levelHistory.value = [
      {
        ...
        fromLevel: 'yellow',
        toLevel: 'orange',
        reason: '学生近期情绪波动加剧，出现较明显的消极言论，需提升关注等级。',
        operator: '李老师',
        operatorRole: '辅导员',
        operateTime: '2024-03-15 14:30'
      },
      {
        id: '2',
        fromLevel: null,
        toLevel: 'yellow',
        reason: '心理测评结果显示存在中度焦虑和抑郁倾向，建档关注。',
        operator: '王咨询师',
        operatorRole: '心理咨询师',
        operateTime: '2024-03-01 10:00'
      }
    ]
  }
  loading.value = false
...
const submitAdjust = async () => {
  ...
  try {
    const res = await request.post('/api/crisis/update-level', {
      id: currentCase.value.id,
      level: adjustForm.value.newLevel,
