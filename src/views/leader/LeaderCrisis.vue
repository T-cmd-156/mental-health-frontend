...
const loadData = async () => {
  ...
  try {
    ...
    const res = await request.get('/api/center/approval/crisis/list', { params })
    if (res.code === 200) {
...
const viewDetail = async (row) => {
  ...
  try {
    const res = await request.get('/api/crisis/detail', { params: { id: row.id } })
    if (res.code === 200) {
...
const submitApprove = async () => {
  ...
  try {
    await request.post('/api/center/approval/crisis/approve', {
...
const submitSupervise = async () => {
  ...
  submitting.value = true
  try {
    await request.post('/api/crisis/intervention/add', {
      id: currentCrisis.value.id,
      ...superviseForm.value,
    })
