...
const loadData = async () => {
  ...
  try {
    ...
    const res = await request.get('/api/leader/crisis', { params })
    if (res.code === 200) {
...
const viewDetail = async (row) => {
  ...
  try {
    const res = await request.get(`/api/leader/crisis/${row.id}`)
    if (res.code === 200) {
...
const submitApprove = async () => {
  ...
  try {
    await request.post('/api/leader/crisis/approve', {
...
const submitSupervise = async () => {
  ...
  submitting.value = true
  try {
    await request.post('/api/leader/crisis/supervise', {
      id: currentCrisis.value.id,
      ...superviseForm.value,
    })
