...
const loadReports = async () => {
  loading.value = true
  try {
    const res = await request.get('/api/leader/reports', {
      params: {
  ...
  try {
    const res = await request.get('/api/leader/college-reports', { params: { year: selectedYear.value, month: '03' } })
    if (res.code === 200) {
...
const viewReport = async (row) => {
  ...
  try {
    const res = await request.get(`/api/leader/reports/${row.id}`)
    if (res.code === 200) {
