...
const loadReports = async () => {
  loading.value = true
  try {
    const res = await request.get('/api/report/list', {
      params: {
  ...
  try {
    const res = await request.get('/api/report/history', { params: { year: selectedYear.value, month: '03', scope: 'college' } })
    if (res.code === 200) {
...
const viewReport = async (row) => {
  ...
  try {
    const res = await request.post('/api/report/preview', { reportId: row.id })
    if (res.code === 200) {
