  ...
  <el-card>
    <div v-if="appointment">
      ...
      <el-descriptions :column="2" border style="margin-bottom:20px">
        ...
        <el-descriptions-item label="学生ID">{{ appointment.student_id }}</el-descriptions-item>
        <el-descriptions-item label="咨询师">{{ appointment.consultant_name }}</el-descriptions-item>
        <el-descriptions-item label="预约日期">{{ appointment.date }}</el-descriptions-item>
        <el-descriptions-item label="预约时间">{{ appointment.start_time }}</el-descriptions-item>
...
const handleReject = async () => {
  ...
  const student_id = appointment.value.student_id
  try {
    ...
    // 模拟通知
    try { window.dispatchEvent(new CustomEvent('mock-message', { detail: { type: 'appointment_rejected', id: appointment.value.id, studentId: student_id } })) } catch(e){}
  } catch (e) {
    ElMessage.error('操作失败')
  }
}
...
const handleRenew = async () => {
  ...
  try {
    const res = await createAppointmentForStudentAsync({
      student_id: appointment.value.student_id,
      counselor_id: appointment.value.counselor_id,
      consultant_name: appointment.value.consultant_name || '',
      date: nextWeekDate,
      start_time: appointment.value.start_time,
      end_time: appointment.value.end_time,
    })
    ElMessage.success(`已创建下周预约：${nextWeekDate} ${appointment.value.start_time}`)
    // 模拟通知
