<template>
  <div class="admin">
    ...
    <header class="top">
      ...
      <div class="top-user">
        <router-link :to="staffDashboardPath" class="staff-avatar-link" title="进入工作台">
          <el-avatar :size="34">{{ staffInitial }}</el-avatar>
        </router-link>
        <span class="staff-name">{{ staffDisplayName }}</span>
        <button type="button" class="staff-logout" @click="staffLogout">退出</button>
      </div>
    </header>
...
<aside class="menu">
  ...
  <template v-else>
    ...
    <!-- 朋辈互助审核：心理中心 -->
    <div class="item" v-if="role === 'center'" :class="{ active: currentPath === 'peer-forum' }" @click="go('peer-forum')">朋辈互助审核</div>
...
import { clearAuthTokens } from '../../request.js'
import {
  Bell,
  Odometer,
  Clock,
...
onMounted(async () => {
  try {
    ...
    const n = res?.data
    unreadCount.value = typeof n === 'number' && Number.isFinite(n) ? n : Number(n) || 0
  } catch (_) {
    unreadCount.value = 0
  }
})
...
const staffDisplayName = computed(() => {
  void route.fullPath
  return localStorage.getItem('user_name') || localStorage.getItem('User_name') || '用户'
})

const staffInitial = computed(() => {
  const s = staffDisplayName.value.trim()
  return s ? s.slice(0, 1) : '?'
})

const staffDashboardPath = computed(() => {
  void route.fullPath
  const r = localStorage.getItem('admin_role') || localStorage.getItem('user_role')
  if (r === 'tutor') return '/admin/tutor-workbench'
  if (r === 'college') return '/admin/college-workbench'
  if (r === 'leader') return '/admin/leader-workbench'
...
const menuEntries = computed(() => [
  ...
  { path: 'activity-manage', label: '团体活动管理', icon: Calendar, show: role.value === 'counselor' || role.value === 'center' || role.value === 'tutor' },
