...
async function loadStats() {
  try {
    const [crisisRes, leaveRes, msgRes] = await Promise.all([
      request.get('/api/crisis/list', { params: { status: 'pending' } }),
      request.get('/api/leave/approval/list', { params: { status: 'pending' } }),
      getUnreadCount(),
    ])
    ...
    stats.value = { crisisPending: 2, leavePending: 2, unreadCount: 3, appointmentToday: 5 }
  }
}
