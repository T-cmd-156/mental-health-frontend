<template>
  <div class="admin-leave">
    <div class="page-head">
      <div class="head-main">
        <h2>请假审批</h2>
        <p class="page-desc">咨询师请假申请审批，一键续约、自动关时段。</p>
      </div>
...
async function loadData() {
  try {
    const res = await request.get('/api/leave/approval/list', { params: { status: 'pending' } })
...
async function loadProcessed() {
  try {
    const res = await request.get('/api/leave/approval/list', { params: { status: filterStatus.value } })
    if (res?.code === 200 && Array.isArray(res.data)) {
...
function approve(row) {
  ElMessageBox.confirm('通过后将自动关闭该时段排班，学生无法预约。确定通过？', '确认通过', {
    ...
    .then(() => {
      request
        .post('/api/leave/approve', { id: row.id })
        .then((res) => {
          if (res?.code === 200) {
...
function confirmReject() {
  ...
  request
    .post('/api/leave/reject', { id: row.id, reason: rejectReason.value })
    .then((res) => {
      if (res?.code === 200) {
