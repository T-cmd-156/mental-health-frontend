...
const loadPendingList = async () => {
  loading.value = true
  try {
    const res = await request.get('/api/crisis/pending')
    if (res.code === 200) {
...
const loadProcessingList = async () => {
  loading.value = true
  try {
    const res = await request.get('/api/crisis/processing')
    if (res.code === 200) {
...
const loadClosedList = async () => {
  loading.value = true
  try {
    const res = await request.get('/api/crisis/closed', {
      params: {
...
const submitApprove = async () => {
  ...
  try {
    await request.post('/api/crisis/approve', approveForm.value)
...
const submitReject = async () => {
  ...
  submitting.value = true
  try {
    await request.post('/api/crisis/reject', rejectForm.value)
