...
const loadPendingList = async () => {
  loading.value = true
  try {
    const res = await request.get('/api/crisis/list', { params: { status: 'pending' } })
    if (res.code === 200) {
...
const loadProcessingList = async () => {
  loading.value = true
  try {
    const res = await request.get('/api/crisis/list', { params: { status: 'processing' } })
    if (res.code === 200) {
...
const loadClosedList = async () => {
  loading.value = true
  try {
    const res = await request.get('/api/crisis/list', {
      params: {
...
const submitApprove = async () => {
  ...
  try {
    await request.post('/api/center/approval/crisis/approve', approveForm.value)
...
const submitReject = async () => {
  ...
  submitting.value = true
  try {
    await request.post('/api/center/approval/crisis/reject', rejectForm.value)
