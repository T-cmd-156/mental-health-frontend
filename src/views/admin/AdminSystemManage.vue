        ...
        <el-checkbox-group v-model="checkedPerms">
          <div class="perm-group">
            <div v-for="g in permGroups" :key="g.label" class="perm-group-item">
              ...
              <el-checkbox v-for="p in g.items" :key="p.value" :label="p.value">{{ p.label }}</el-checkbox>
            </div>
          </div>
        </el-checkbox-group>
      </div>
...
async function loadParams() {
  try {
    const res = await request.get('/api/admin/config/system')
...
function saveParams() {
  request.put('/api/admin/config/system', systemParams).then((res) => {
    if (res?.code === 200) {
