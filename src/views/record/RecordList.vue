        ...
        <el-table-column label="操作" width="160" fixed="right">
          <template #default="{ row }">
            ...
            <el-button
              ...
              link
              size="small"
              :disabled="row.record_status === 'submitted'"
              @click="edit(row.id)"
            >
              编辑
            </el-button>
...
const counselorId = localStorage.getItem('user_id') || localStorage.getItem('counselorId') || ''
...
function getStatusType(status: string) {
  if (status === 'submitted' || status === '已审核') return 'success'
  if (status === 'pending' || status === 'filled' || status === '未审核') return 'warning'
  return 'info'
}
