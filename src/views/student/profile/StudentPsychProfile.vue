...
const loadAssessments = async () => {
  loading.value = true
  try {
    const res = await request.get('/api/center/archive/student/detail', {
      params: {
...
const loadConsultations = async () => {
  try {
    const res = await request.get('/api/center/archive/student/detail', {
      params: { studentId: studentInfo.value.id, section: 'consultations' }
    })
    if (res.code === 200) {
...
const loadCrisis = async () => {
  try {
    const res = await request.get('/api/center/archive/student/detail', {
      params: { studentId: studentInfo.value.id, section: 'crisis' }
    })
    if (res.code === 200) {
...
const loadTracking = async () => {
  try {
    const res = await request.get('/api/center/archive/student/detail', {
      params: { studentId: studentInfo.value.id, section: 'tracking' }
    })
    if (res.code === 200) {
...
const loadWarningHistory = async () => {
  try {
    const res = await request.get('/api/center/archive/student/detail', {
      params: { studentId: studentInfo.value.id, section: 'warning-history' }
    })
    if (res.code === 200) {
