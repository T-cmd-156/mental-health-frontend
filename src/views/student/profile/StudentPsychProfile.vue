...
const loadAssessments = async () => {
  loading.value = true
  try {
    const res = await request.get('/api/student/assessments', {
      params: {
...
const loadConsultations = async () => {
  try {
    const res = await request.get('/api/student/consultations', {
      params: { studentId: studentInfo.value.id }
    })
    if (res.code === 200) {
...
const loadCrisis = async () => {
  try {
    const res = await request.get('/api/student/crisis', {
      params: { studentId: studentInfo.value.id }
    })
    if (res.code === 200) {
...
const loadTracking = async () => {
  try {
    const res = await request.get('/api/student/tracking', {
      params: { studentId: studentInfo.value.id }
    })
    if (res.code === 200) {
...
const loadWarningHistory = async () => {
  try {
    const res = await request.get('/api/student/warning-history', {
      params: { studentId: studentInfo.value.id }
    })
    if (res.code === 200) {
