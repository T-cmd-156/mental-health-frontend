...
const loadReportData = async () => {
  ...
  try {
    const res = await request.get('/api/tutor/report', { params: { month: selectedMonth.value } })
...
const loadHistory = async () => {
  try {
    const res = await request.get('/api/tutor/report/history')
    if (res.code === 200) {
...
const saveDraft = async () => {
  saving.value = true
  try {
    await request.post('/api/tutor/report/draft', {
...
const submitReport = async () => {
  ...
  try {
    await request.post('/api/tutor/report/submit', {
      month: selectedMonth.value,
      ...form.value,
    })
