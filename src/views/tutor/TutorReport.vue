...
const loadReportData = async () => {
  ...
  try {
    const res = await request.get('/api/report/list', { params: { month: selectedMonth.value, scope: 'tutor' } })
...
const loadHistory = async () => {
  try {
    const res = await request.get('/api/report/history', { params: { scope: 'tutor' } })
    if (res.code === 200) {
...
const saveDraft = async () => {
  saving.value = true
  try {
    await request.post('/api/report/preview', {
...
const submitReport = async () => {
  ...
  try {
    await request.post('/api/report/generate', {
      month: selectedMonth.value,
      ...form.value,
    })
