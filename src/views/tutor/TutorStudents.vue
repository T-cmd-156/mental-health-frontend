...
const loadData = async () => {
  ...
  try {
    ...
    const res = await request.get('/api/tutor/students', { params })
    if (res.code === 200) {
...
const viewDetail = async (student) => {
  ...
  try {
    const res = await request.get(`/api/tutor/students/${student.id}`)
    if (res.code === 200) {
...
const submitRecord = async () => {
  ...
  try {
    await request.post('/api/tutor/students/record', {
