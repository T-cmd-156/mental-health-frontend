...
const loadData = async () => {
  ...
  try {
    ...
    const res = await request.get('/api/center/archive/students', { params })
    if (res.code === 200) {
...
const viewDetail = async (student) => {
  ...
  try {
    const res = await request.get('/api/center/archive/student/detail', { params: { id: student.id } })
    if (res.code === 200) {
...
const submitRecord = async () => {
  ...
  try {
    await request.post('/api/conversation/save', {
