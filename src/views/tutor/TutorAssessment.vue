...
const loadData = async () => {
  ...
  try {
    ...
    const res = await request.get('/api/tutor/assessments', { params })
    if (res.code === 200) {
...
const loadClassList = async () => {
  try {
    const res = await request.get('/api/tutor/classes')
    if (res.code === 200) {
...
const remindStudent = async (row) => {
  try {
    await request.post('/api/tutor/assessment/remind', { studentId: row.studentId })
