...
const loadData = async () => {
  ...
  try {
    ...
    const res = await request.get('/api/assessment/my-list', { params })
    if (res.code === 200) {
      assessmentList.value = res.data?.list || res.data || []
      totalCount.value = res.data?.total || assessmentList.value.length
      if (res.data?.stats) {
...
const loadClassList = async () => {
  try {
    const res = await request.get('/api/common/colleges')
    if (res.code === 200) {
      classList.value = res.data?.map((c) => ({ label: c.name, value: c.id })) || []
    }
  } catch (e) {
    classList.value = [
...
const viewResult = async (row) => {
  try {
    const res = await request.get(`/api/assessment/result?id=${row.id}`)
    if (res.code === 200) {
...
const remindStudent = async (row) => {
  try {
    await request.post('/api/message/send', { studentId: row.studentId, type: 'assessment-remind' })
