...
const loadData = async () => {
  try {
    const res = await request.get('/api/college/info')
    if (res.code === 200) {
  ...
  try {
    const res = await request.get('/api/college/tutors')
    if (res.code === 200) {
  ...
  try {
    const res = await request.get('/api/college/departments')
    if (res.code === 200) {
...
const saveTutor = async () => {
  saving.value = true
  try {
    if (isEditTutor.value) {
      await request.put(`/api/college/tutors/${tutorForm.value.id}`, tutorForm.value)
    } else {
      await request.post('/api/college/tutors', tutorForm.value)
    }
...
const saveDept = async () => {
  ...
  try {
    if (isEditDept.value) {
      await request.put(`/api/college/departments/${deptForm.value.id}`, deptForm.value)
    } else {
      await request.post('/api/college/departments', deptForm.value)
    }
    ElMessage.success('保存成功')
...
const deleteDept = async (row) => {
  try {
    ...
    await request.delete(`/api/college/departments/${row.id}`)
